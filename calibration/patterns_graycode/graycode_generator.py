...
def generate_graycode_patterns(width: int, height: int, out_dir: str | None = None):
    ...
    Returns
    -------
    patterns_x : list[tuple[str, pygame.Surface]]
        List of (label, surface) for X bits (on/off alternating).
    patterns_y : list[tuple[str, pygame.Surface]]
        List of (label, surface) for Y bits (on/off alternating).
    bits_x, bits_y : int
    ...
    for bit in range(bits_x):
        ...
        patterns_x.append((f"x_bit{bit:02d}_on", _surface_from_mask(stripe)))
        patterns_x.append((f"x_bit{bit:02d}_off", _surface_from_mask(stripe_inv)))
    ...
    for bit in range(bits_y):
        ...
        patterns_y.append((f"y_bit{bit:02d}_on", _surface_from_mask(stripe)))
        patterns_y.append((f"y_bit{bit:02d}_off", _surface_from_mask(stripe_inv)))
