...
class FringeServer:
    ...
    def _configure_routes(self) -> None:
        ...
        async def compute_phase(run_id: str):
            try:
                images = self.run_store.load_captures(run_id)
                ...
                ref = self.run_store.load_reference_image(run_id)
                roi_cfg = ObjectRoiConfig()
                roi_res = detect_object_roi(ref, roi_cfg)
                from dataclasses import asdict as _asdict
                self.run_store.save_roi(run_id, roi_res.roi_mask, roi_res.bbox, {"cfg": _asdict(roi_cfg), "debug": roi_res.debug})
                ...
                self.run_store.save_phase_outputs(run_id, result)
                # Save valid-in-ROI mask and overlays
                ...
                try:
                    ...
                    pass
                return {"ok": True, "run_id": run_id, "stats": result.debug}
    ...
    def _build_params(self, payload: Dict[str, Any]) -> ScanParams:
        ...
        params = ScanParams(
            ...
            frequency=float(payload.get("frequency", defaults.get("frequency", 8.0))),
            ...
            iso=payload.get("iso"),
            brightness_offset=float(payload.get("brightness_offset", defaults.get("brightness_offset", 0.5))),
            contrast=float(payload.get("contrast", defaults.get("contrast", 1.0))),
        )
        return params
    ...
    def _build_phase_thresholds(self) -> PhaseThresholds:
        ...
        return PhaseThresholds(
            sat_low=float(cfg.get("sat_low", 5)),
            sat_high=float(cfg.get("sat_high", 250)),
            B_thresh=float(cfg.get("B_thresh", 10)),
            A_min=float(cfg.get("A_min", 0)),
